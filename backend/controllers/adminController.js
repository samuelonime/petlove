...
class AdminController {
  ...
  static async login(req, res) {
    try {
      ...
      if (!admin) {
        await this.logSecurity('admin_login_failed', req, { email, reason: 'not_found' });
      ...
      if (!isPasswordValid) {
        ...
        await this.logSecurity('admin_login_failed', req, { email, reason: 'wrong_password' });
      ...
      if (admin.two_factor_enabled) {
        ...
        if (!verified) {
          await this.logSecurity('admin_2fa_failed', req, { email });
      ...
      await this.logAction(admin.id, 'ADMIN_LOGIN', req);
...
module.exports = AdminController;        process.env.JWT_SECRET,
        { expiresIn: '15m' }
      );
